...
#'@param newdata: this is a new data set for which it is desired to make predictions. 
...
# allow specification of grouping variable (id.name)

mixoutsamp=function(model,newdata,id.name="patid"){
...
#   print(G)
  ...
  for(i in 1:length(split.obs.ids)){
	...
	# if (i == 1) {
		# print(model$modelStruct$varStruct)
	# }
    ...
    if(is.null(model$modelStruct$varStruct)==1){
      ...
      resid.var.mat=matrix(resid.var,nrow=dim(corr.mat)[1],ncol=dim(corr.mat)[1])
		...
		# print(resid.var[as.character(ytype[oids.list])])
	...
	# 	print(as.matrix(G))
	# 	print(t.z.mat.oids)
	# 	print(G%*%t.z.mat.oids)
	# }
    
    reffects.individual[i,] = (G%*%t.z.mat.oids) %*% solve(((z.mat.oids %*% G %*% t.z.mat.oids) + (resid.var.mat * corr.mat))) %*% (y[oids.list]-xb[oids.list])
	  
	  
    reffects[pids.list]=zstar.mat[pids.list,]%*%t(reffects.individual[i,,drop=FALSE])
	# if (i == 1) {
		# print(reffects.individual[i,])
		# print(zstar.mat[pids.list,])
		# print(reffects[pids.list])
	# }
  }
  ...
  return(list(preddata=preddata,random=random))
